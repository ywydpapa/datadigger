...
"""
# 실제 사용 시 기존 전체 HTML_SNIPPET 그대로 넣으세요.
...
def download_image(session, img_url, save_path):
    resp = session.get(img_url, headers=BASE_HEADERS, timeout=30, stream=True)
...
def parse_club_page(html, url=""):
    ...
    # 클럽 3역 + 사진
    items = soup.select("ul.img_list li")
    for li in items:
        ...
        if role in TARGET_ROLES:
            ...
            if img and img.get("src"):
                abs_img_url = urljoin(url, img.get("src").strip())
                result[f"{role}사진URL"] = abs_img_url
...
def save_role_photo(session, root_dir, club_name, role, person_name, photo_url, use_club_prefix=False):
    if not person_name or not photo_url:
        return "", "SKIPPED", "이름 또는 사진 URL 없음"

    role_dir = os.path.join(root_dir, role)
    ensure_dir(role_dir)

    safe_person_name = safe_filename(person_name)
    safe_club_name = safe_filename(club_name)
    ext = get_image_extension_from_url(photo_url)

    if use_club_prefix:
        filename = f"{safe_club_name}_{safe_person_name}{ext}"
    else:
        filename = f"{safe_person_name}{ext}"

    save_path = os.path.join(role_dir, filename)

    if os.path.exists(save_path):
        return save_path, "EXISTS", "이미 파일이 존재함"

    download_image(session, photo_url, save_path)
    return save_path, "DOWNLOADED", "다운로드 성공"
...
def main():
    ...
    for idx, item in enumerate(links, start=1):
        ...
        try:
            ...
            # 회장 사진
            p1, s1, m1 = save_role_photo(
                session=session,
                root_dir=photo_root_dir,
                club_name=club_name,
                role="회장",
                person_name=row["회장"],
                photo_url=row["회장사진URL"],
                use_club_prefix=False
            )
            row["회장사진파일"] = p1
            row["회장사진상태"] = s1
            row["회장사진메시지"] = m1
            ...
            # 총무 사진
            p2, s2, m2 = save_role_photo(
                session=session,
                root_dir=photo_root_dir,
                club_name=club_name,
                role="총무",
                person_name=row["총무"],
                photo_url=row["총무사진URL"],
                use_club_prefix=False
            )
            row["총무사진파일"] = p2
            row["총무사진상태"] = s2
            ...
            # 재무 사진
            p3, s3, m3 = save_role_photo(
                session=session,
                root_dir=photo_root_dir,
                club_name=club_name,
                role="재무",
                person_name=row["재무"],
                photo_url=row["재무사진URL"],
                use_club_prefix=False
            )
            row["재무사진파일"] = p3
            row["재무사진상태"] = s3
