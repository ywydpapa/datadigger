...
def get_rank_no(role_str: str) -> int:
    """
    직책 문자열을 분석하여 rankNo를 반환합니다.
    사무부총장: 6, 재무부총장: 7
    """
    role = role_str.strip()
    if "재무" in role:
        ...
        try:
            async with engine.begin() as conn:
                for idx, row in enumerate(reader, start=2):
                    ...
                    rank_no = get_rank_no(role_str)  # 직책에 따른 rankNo 동적 할당
                    ...
                    try:
                        ...
                        # 같은 연도에 여러 부총장이 있으므로 memberNo까지 포함하여 중복 검사
                        check_result = await conn.execute(
                            text(
                                "SELECT dstaffNo FROM yk_distStaff WHERE periodNo = :period_no AND rankNo = :rank_no AND memberNo = :member_no LIMIT 1"
                            ),
                            {"period_no": period_no, "rank_no": rank_no, "member_no": member_no}
                        )
                        ...
                        if existing_row:
                            ...
                            updated += 1
                            print(
                                f"[UPDATE] {idx}행: {period_str} {member_name} ({role_str}, rankNo: {rank_no}) 데이터 업데이트 완료")
                        else:
                            await conn.execute(
                                text(
                                    """
                            ...
                            inserted += 1
                            print(
                                f"[INSERT] {idx}행: {period_str} {member_name} ({role_str}, rankNo: {rank_no}) 데이터 추가 완료")
                    except Exception as e:
